...
#define FLASHINFER_CUDA_CALL(func, ...)                                        \
    {                                                                          \
        hipError_t e = (func);                                                 \
        if (e != hipSuccess) {                                                 \
            std::cerr << "HIP Error: " << hipGetErrorString(e) << " (" << e    \
...
DebugPrintCUDAArray(T *device_ptr, size_t size, std::string prefix = "")
{
    ...
    std::cout << prefix;
    hipMemcpy(host_array.data(), device_ptr, size * sizeof(T),
...
/// @brief  Perform Simple Subtraction
/// @param x Input param X
/// @param y Input param y
/// @return Returns x - y if x > y; else 0;
