...
__global__ void RMSNormKernel(T *__restrict__ input,
...
#pragma unroll
        ...
        if ((i * num_threads + thread_id) * VEC_SIZE < d) {
            residual_vec.store(residual + bx * stride_residual +
                               i * num_threads * VEC_SIZE +
                               thread_id * VEC_SIZE);
            x_vec.store(smem_x + i * num_threads * VEC_SIZE +
                        thread_id * VEC_SIZE);
        }
    }
...
#pragma unroll
    ...
    for (uint32_t i = 0; i < rounds; i++) {
        ...
        if ((i * num_threads + thread_id) * VEC_SIZE < d) {
            weight_vec.load(weight + i * num_threads * VEC_SIZE +
                            thread_id * VEC_SIZE);
            x_vec.load(smem_x + i * num_threads * VEC_SIZE +
                       thread_id * VEC_SIZE);
        }
#pragma unroll
        for (uint32_t j = 0; j < VEC_SIZE; j++) {
            input_vec[j] =
                x_vec[j] * rms_rcp * (weight_bias + float(weight_vec[j]));
        }
        if ((i * num_threads + thread_id) * VEC_SIZE < d) {
            input_vec.store(input + bx * stride_input +
                            i * num_threads * VEC_SIZE + thread_id * VEC_SIZE);
        }
    }
#if (__CUDACC_VER_MAJOR__ >= 12 && defined(__CUDA_ARCH__) &&                   \
...
template <typename T>
hipError_t FusedAddRMSNorm(T *input,
                           T *residual,
                           T *weight,
                           uint32_t batch_size,
                           uint32_t d,
...
template <typename T>
hipError_t GemmaRMSNorm(T *input,
                        T *weight,
                        T *output,
                        uint32_t batch_size,
                        uint32_t d,
...
template <typename T>
hipError_t GemmaFusedAddRMSNorm(T *input,
                                T *residual,
                                T *weight,
                                uint32_t batch_size,
                                uint32_t d,
